...
def generate_progression(element, step, progression_length):
    progression = [str(element + step) for _ in range(progression_length + 1)]
    return progression


def create_progression_question_and_answer():
    element = random.randint(MIN_RANDOM_ELEMENT, MAX_RANDOM_ELEMENT) 
    step = random.randint(MIN_RANDOM_STEP, MAX_RANDOM_STEP)
    progression_length = random.randint(MIN_PROGRESSION_LENGTH, MAX_PROGRESSION_LENGTH)
    progression = generate_progression(element, step, progression_length)
